...
* **Task 1.4:** Create an Angular HTTP Service to fetch public data from the Golang backend (`GET /api/courses` and `GET /api/instructors/schedule`).
...
* **Task 2.2 (Backend):** Define the Golang Course struct and set up the database connection.
* **Task 2.3 (Backend):** Implement the Golang REST API route handlers for CRUD operations (`GET`, `POST`, `PUT`, `DELETE` on `/api/courses`).
...
* **Task 2.6 (Integration):** Connect the Angular form and data grid to the Golang backend API to fully enable data management.
...
* **Task 3.2 (Backend):** Define the Golang Instructor and Schedule structs.
* **Task 3.3 (Backend):** Implement the Golang REST API handlers for Instructor CRUD (`/api/instructors`).
* **Task 3.4 (Backend):** Implement the Golang REST API handlers for updating the schedule matrix (`GET` and `POST`/`PUT` on `/api/instructors/{id}/schedule`).
...
* **Task 4.2 (Backend):** Define the Golang Mechanism struct.
* **Task 4.3 (Backend):** Implement the Golang REST API handlers for CRUD operations (`GET`, `POST`, `PUT`, `DELETE` on `/api/mechanisms`).
...
* **Task 4.6 (Integration):** Connect the Angular mechanism UI to the Golang API.
